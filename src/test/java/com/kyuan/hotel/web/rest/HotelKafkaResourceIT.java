...
import com.kyuan.hotel.config.KafkaProperties;
...
class HotelKafkaResourceIT {
    ...
    void setup() {
        ...
        HotelKafkaResource kafkaResource = new HotelKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/hotel-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/hotel-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
